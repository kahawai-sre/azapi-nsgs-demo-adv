...
module "nsg-subnetassociation-azapi" {
  ...
  #// asper limitation https://github.com/Azure/terraform-provider-azapi/issues/204
  nsg_id     = each.value.associated_state == true ? module.nsg-azapi-new[each.value.nsg_name].nsg_id : "" #// <<<=== check associated_state flag. If associated_state = true, do the assignment, else, clear the assignment (for Day2 ops where want to clear subnet => NSG link)
  subnet_id  = each.value.subnet_id
  depends_on = [module.nsg-azapi-new]
}
